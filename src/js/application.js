      ...
      articles.forEach(article => {
        ...
        row.innerHTML = `
          ...
          <td class="actions">
            ...
            <a href="#" class="uk-icon-link" uk-icon="trash" uk-tooltip="Supprimer" onclick="deleteArticle('${encodeURIComponent(article.name)}');"></a>
          </td>
        `;
...
function addArticle(name, code) {
  // Retirer les guillemets indésirables
  name = name.replace(/"/g, "");
  code = code.replace(/"/g, "");
...
function saveArticle(name, newCode) {
  ...
  xhr.send(`name="${name}",new_code="${newCode}"&`);
}
